...
| <a name="input_ip_range_filter"></a> [ip\_range\_filter](#input\_ip\_range\_filter) | Cosmos `ipRules` set. When a private endpoint is configured AND<br/>`public_network_access_enabled = true`, Cosmos enters a default<br/>"restricted public" mode where public traffic is dropped unless<br/>explicitly allowed via this set. The special magic value `0.0.0.0`<br/>permits traffic originating from any Azure datacenter — used in<br/>dev so the Container Apps Environment's egress NAT (a public<br/>Azure-allocated IP) can reach Cosmos for the indexer's change-feed<br/>listener. Empty set keeps the default-restrictive posture (PE-only<br/>+ named IP allowlist). Bare IP literals or CIDRs are also accepted. | `set(string)` | `[]` | no |
