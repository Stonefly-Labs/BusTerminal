...
# `-backend-config=` flags (see `.github/workflows/cd-dev.yml`):
#   - resource_group_name
#   - storage_account_name
#   - container_name
#   - subscription_id
#   - tenant_id
#   - client_id  (the dev pipeline managed identity from platform-bootstrap)
#
# Local developer workflows MAY initialize this with `-backend=false` to keep
# state local; remote state is required only for shared / CI usage (FR-082).
terraform {
  backend "azurerm" {
