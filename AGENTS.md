...
Codex MUST consult the relevant MCP/tool rather than relying on training data when working in these domains:
...
When multiple sources of truth conflict (training data vs MCP vs local code), **the MCP wins**. When a task touches any domain above and Codex hasn't consulted the relevant MCP, that's a process failure — flag it and consult before proceeding.
