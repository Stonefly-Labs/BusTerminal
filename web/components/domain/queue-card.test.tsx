...
describe("QueueCard", () => {
  it("is axe-clean across statuses", async () => {
    const { container } = render(
      <div>
        <QueueCard queue={BASE} />
        <QueueCard queue={{ ...BASE, id: "q-2", status: "idle" }} />
        <QueueCard queue={{ ...BASE, id: "q-3", status: "error", deadLetterCount: 14 }} />
        <QueueCard queue={{ ...BASE, id: "q-4", status: "dead-lettered", deadLetterCount: 3 }} />
      </div>,
    );
  ...
  it("truncates oversized queue names and exposes the full value to assistive tech", () => {
    render(<QueueCard queue={{ ...BASE, name: LONG_NAME }} />);
