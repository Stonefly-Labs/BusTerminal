...
- [X] T135 [P] [US6] Create `api/BusTerminal.Api.Tests/Unit/Domain/ExtensionsTests.cs` covering: structured-object round-trip; primitive round-trip; multi-vendor coexistence with same suffix; `__indexable` opt-out marker preservation; rejection of malformed keys at construction.
- [X] T136 [P] [US6] Create `api/BusTerminal.Api.Tests/Unit/Validation/Rules/ExtensionKeyFormatRuleTests.cs` covering namespaced-key pass, malformed-key Warning, `__indexable` ignored by the rule.
...
- [X] T137 [US7] Extend `ICanonicalResourceStore` and `CosmosCanonicalResourceStore` (T049 / T050) with `QueryAsync(ResourceQuery.InEnvironment(EnvironmentClassification env))` returning only resources whose `Environments` array contains the requested classification. Cosmos query: `WHERE ARRAY_CONTAINS(c.environments, @env)`. *(Already wired during Phase 2 — `ResourceQuery.InEnvironment` variant exists in `ResourceQuery.cs` and `BuildQuery` in `CosmosCanonicalResourceStore` emits `ARRAY_CONTAINS(c.environments, @env)` with the env value as the parameter. No additional Cosmos indexing-policy change needed: the `resources` container indexes everything except `/extensions/*` and `_etag`, so `c.environments` is served by the default index.)*
- [X] T138 [US7] Create `api/BusTerminal.Api/Fixtures/05-environments.json` (new envelope file, additive — patch-style per T134) ensuring exactly one Queue carries all six minimum environments as a single document, and at least two resources carry a custom environment value (e.g., `"training"`) to exercise the extensibility path. *(Patch overlays Queue ...004 with all six minimum environments (development/test/qa/staging/production/disasterRecovery) on a single document and preserves the T134 extensions block so the SC-006 fixture state survives. Broker ...003 and Policy ...00c each carry `["production", "training"]` for the custom-vocabulary path. Glob-included in the test csproj — no project changes needed.)*
- [X] T139 [P] [US7] Create `api/BusTerminal.Api.Tests/Unit/Domain/EnvironmentClassificationTests.cs` covering: known-case serialization; custom-case serialization; round-trip preservation. *(12 facts: per-known-case wire-form + round-trip theories, custom-case serialize + round-trip, mixed array round-trip, empty/null guard, and known-vs-custom equality. Asserts the JSON converter emits a bare camelCase string for known cases and passes the raw string through for custom values per FR-017.)*
- [X] T140 [US7] Create `api/BusTerminal.Api.Tests/Integration/Persistence/EnvironmentFilterIntegrationTests.cs` covering: load fixtures; query by Production; assert no duplicate logical resources per environment; query by custom environment value. **SC-007 evidence.** *(4 facts using the CosmosEmulator collection: (1) single Queue document carries all six minimum environments after the patch overlay; (2) `InEnvironment(Production)` returns each logical resource at most once; (3) `InEnvironment(DisasterRecovery)` only surfaces the six-env queue with unique IDs; (4) custom env "training" is queryable end-to-end and returns Broker + Policy. All 4 pass against the live emulator; full 29 integration + 335 total suite green.)*
...
- [X] T141 [US8] Extend `IResourceSerializer` (T044) with YAML methods: `string SerializeToYaml(Resource)`, `Resource DeserializeFromYaml(string)`, `string SerializeEnvelopeToYaml(ImportExportEnvelope)`, `ImportExportEnvelope DeserializeEnvelopeFromYaml(string)`. *(Default-method throws replaced with abstract members; `JsonResourceSerializer` explicitly throws on the YAML methods with a pointer to `YamlResourceSerializer` so wiring bugs are loud rather than silently emitting JSON.)*
- [X] T142 [US8] Create `api/BusTerminal.Api/Domain/Serialization/YamlResourceSerializer.cs` implementing the YAML methods of `IResourceSerializer` using YamlDotNet 16.x. Consult context7 MCP for current YamlDotNet builder/converter patterns. The logical shape MUST match JSON exactly (envelope structure, extensions, polymorphic discriminator). *(Strategy: tunnel YAML through the JSON pipeline — Resource → JSON via `JsonResourceSerializer` → `JsonElement` tree → `YamlNode` tree (preserving strings as DoubleQuoted, numbers/bools/null as Plain) → `YamlStream.Save`. Reverse path on input. Context7 MCP confirmed `SerializerBuilder.JsonCompatible()` exists in 16.3 but the explicit tree-to-tree conversion controls scalar-style precisely and avoids YamlDotNet's implicit type-coercion of plain scalars. Registered as a DI singleton alongside `JsonResourceSerializer` in `CosmosConfigurationExtensions`. Build clean, 17 unit tests green.)*
- [X] T143 [US8] Implement the `tools/load-fixtures` `export` subcommand: queries every resource (including soft-deleted via `--include-deleted` flag) and every relationship, optionally queries the change-event log via `--include-change-log`, wraps into `ImportExportEnvelope`, serializes via `--format json|yaml`, writes to `--output <path>`. FR-016. *(Added `IChangeEventLog.QueryAllAsync` for the cross-partition export scan — keeps the per-resource `QueryAsync` overload focused. `ExportCommand` resolves both JSON and YAML serializers from DI and dispatches on `--format`. Wired into Program.cs dispatcher; usage block updated. Also fixed a pre-existing runtime regression in the CLI: the Cosmos 3.x SDK touches `Newtonsoft.Json` at runtime even with `AzureCosmosDisableNewtonsoftJsonCheck=true` suppressing the build warning, but the load-fixtures CLI's `bin/` never carried the assembly (the test project picked it up transitively through xunit, which is why earlier integration tests didn't surface this). Added an explicit `Newtonsoft.Json` 13.0.3 PackageReference to `LoadFixtures.csproj` — the canonical model still uses System.Text.Json end-to-end; this is a transitive runtime crutch, not a serializer choice.)*
- [X] T144 [US8] Implement the `tools/load-fixtures` `import` subcommand (extension of T080): adds `--conflict-resolution reject|skip|overwrite` flag (default `reject`). Resolution outcome is recorded in audit metadata per Scenario 3. *(Conflict resolution is mode-aware when the flag is not explicitly passed: `--input <file>` defaults to `reject` (US8 disaster-recovery into an empty store, matches quickstart Smoke 7); `--fixtures-dir`/`--fixtures` defaults to `overwrite` (preserves the T134 patch-overlay flow that the quickstart smoke depends on). Explicit `--conflict-resolution` always wins for both modes. Resolution outcome is reported on stdout (`Conflict resolution: ... → X overwritten, Y skipped, Z rejected`) and stamped into `Audit.SourceSystem` via the existing Update path. Exit code 2 when conflicts were rejected; exit code 1 for validation Errors; 0 otherwise. Relationships honor the same policy: `reject` reports each existing edge and short-circuits with exit 2; `skip` keeps the existing edge and notes the skip; `overwrite` is reported as a skip because `ICanonicalResourceStore` has no relationship-update path in v1 — extending it is a follow-up if mutation becomes a real need.)*
- [X] T145 [US8] Update the `tools/load-fixtures` `show` subcommand (T081) to support `--format yaml`. *(Dispatch on `--format` between JSON and YAML serializers; rejects unknown formats with a structured error.)*
- [X] T146 [P] [US8] Create `api/BusTerminal.Api.Tests/Unit/Serialization/YamlRoundTripTests.cs` asserting every first-class type round-trips YAML losslessly (mirror of T087). **SC-009 evidence per-type.** *(17 facts: 14 per-type theories asserting JSON-shape preservation after YAML round-trip; envelope round-trip with relationships placeholder; nested-extension stress test exercising strings-that-look-like-numbers / bools / nulls to validate the scalar-style convention; UnknownResource discriminator survives YAML round-trip. `SerializerFixture` now exposes `YamlSerializer` alongside `Serializer`.)*
- [X] T147 [US8] Create `api/BusTerminal.Api.Tests/Integration/Persistence/ExportImportRoundTripTests.cs` covering: load fixtures → export to JSON → truncate → import → assert byte-equivalence of resource set, relationships (with direction and type), version lineage, ownership references, lifecycle, and extensions. Repeat for YAML format. Repeat with conflict-resolution `reject` against a non-empty store and assert the structured conflict result. **SC-009 final evidence.** *(3 facts in the `CosmosEmulator` collection: (1) JSON envelope round-trip — load 01..05 fixtures, export, truncate, re-import, assert per-resource and per-relationship normalized-JSON equivalence (audit + ConcurrencyToken stripped per spec §183 "byte-meaningful"); (2) YAML envelope round-trip — same flow through `YamlResourceSerializer`; (3) reject semantics — re-import into a populated store produces `HttpStatusCode.Conflict` for each duplicate, matching the load-fixtures CLI `[conflict-rejected]` output path. All 3 pass against the live emulator; full 76 integration suite still green.)*
