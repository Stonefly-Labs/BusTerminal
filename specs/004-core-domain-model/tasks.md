...
- [ ] T010 [P] Create `iac/modules/cosmos-account/versions.tf` pinning `terraform >= 1.10`. Match the existing `azurerm` provider pin in `iac/environments/dev/versions.tf` exactly — do not introduce a major-version bump in this slice. Pin the AVM source explicitly to a specific version (read the currently-published version from the AVM module's GitHub releases via Microsoft Learn MCP).
