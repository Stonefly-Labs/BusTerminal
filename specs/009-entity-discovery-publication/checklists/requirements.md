...
- **Clarification session 2026-06-17**: 4 of 5 question slots used. Resolved: (1) discovery duration budget → ≤ 5 min (SC-005); (2) transient-error retry policy → bounded exp-backoff (FR-021a); (3) concurrent-discovery behavior → coalesce (FR-003); (4) entity↔service association → M:N with `Owner`/`Producer`/`Consumer` role taxonomy (FR-011, FR-011a, FR-022, FR-026; `EntityServiceAssociation` key entity added). Deferred: metadata-edit concurrency control (plan-level — follows platform REST convention).
