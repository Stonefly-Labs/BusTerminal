...
- A subsequent `/speckit-clarify` pass on 2026-05-25 added five more clarifications to the same session block: (Q1c) Application Insights ingestion model — hybrid managed-identity / connection-string-via-KV-secret-ref, resolved a self-contradiction between SC-003 and FR-028; (Q2c) dev networking — provision the full VNet + private endpoints + private DNS zones in dev now with public access kept on, so the destructive retrofit is minimal; (Q3c) workload RBAC tier — forward-looking grants scoped to committed downstream specs, with the explicit role list enumerated in FR-033; (Q4c) deployment identity model — per-environment deployment MI (no shared cross-env identity); (Q5c) diagnostic categories + retention — `allLogs` only (no metrics to LA), 30-day default retention exposed as a per-env tf var. Affected FRs: FR-025, FR-027, FR-028, FR-029, FR-030, FR-031, FR-032, FR-033. Affected SCs: SC-003, SC-006. Affected user-story scenarios: US3 #3, US4 #1 and #3. Key Entities: Deployment Managed Identity refined.
