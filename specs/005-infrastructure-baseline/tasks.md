...
- [X] T121 [US7] Verify each stateful resource module already has `lifecycle { prevent_destroy = true }` where appropriate (Cosmos DB account, KV, ACR, LAW, App Insights, tfstate storage) — add the block where missing. Note: this requires balancing intentional replacements (rare) against accidental destroys (the BT-IAC-007 policy gate is the primary defense; `prevent_destroy` is belt-and-suspenders). **Implemented as**: added `prevent_destroy` to the four directly-owned stateful resources (env RG in dev/test/prod, `azurerm_cosmosdb_account.this`, `azurerm_cosmosdb_sql_database.canonical`, `azurerm_key_vault_secret.app_insights_connection_string`). AVM-wrapped stateful resources (KV, ACR, LAW, App Insights, ACE, workload UAMI, tfstate SA) cannot accept `lifecycle` blocks at the wrapper layer — they rely on BT-IAC-007 as their sole defense, enumerated in `iac/environments/dev/README.md` § "AVM-wrapped stateful resources".
- [X] T122 [US7] Verify `iac/environments/dev/variables.tf` `key_vault_purge_protection_enabled` defaults to `false` (dev) AND `iac/environments/{test,prod}/variables.tf` defaults to `true`; thread the variable into `module.keyvault` per FR-019. **Implemented as**: added `purge_protection_enabled` + `soft_delete_retention_days` to `iac/modules/keyvault/variables.tf` (defaults `true` / `90`, validation range 7-90); replaced the hardcoded values in `iac/modules/keyvault/main.tf` with the new vars; threaded the env-level vars into all three `module.keyvault` invocations. Dev's `terraform.tfvars` set to `true` / `90` (matches deployed reality — Azure forbids disabling purge protection once enabled; rationale documented in the dev README + tfvars header).
- [X] T123 [US7] Verify `iac/platform-bootstrap/main.tf` storage account block has `versioning_enabled = true` and 30-day soft-delete retention per `data-model.md` §4 (already present per research §13; this task confirms). **Verified 2026-05-28**: `versioning_enabled = true`, `delete_retention_policy.days = 30`, `container_delete_retention_policy.days = 30` all present in `iac/platform-bootstrap/main.tf` lines 87-95. No code change required.
- [X] T124 [US7] Document the destructive-change manual approval gate in `iac/environments/dev/README.md` (new file, per US5/T106 convention) referencing the BT-IAC-007 rule and the iac-apply-dev.yml approval step. **Implemented as**: new `iac/environments/dev/README.md` documenting (a) dev posture defaults table, (b) the Azure constraint on dev KV purge protection, (c) the two-layered destructive-change protection (BT-IAC-007 primary + `prevent_destroy` belt-and-suspenders), (d) the enumeration of AVM-wrapped stateful resources that fall back to BT-IAC-007 only, and (e) the intentional-teardown bypass procedure (PR removes `lifecycle` + CI gate approval).
...
- [X] T125 [P] Run `tofu fmt -recursive iac/` from the repo root; commit any formatting changes. **Verified 2026-05-30**: `tofu fmt -check -recursive iac/` exits 0; no changes required.
- [X] T126 [P] Run `terraform-docs markdown table iac/modules/<each-module>/` and commit the regenerated module READMEs (or wire `terraform-docs` as a CI formatting gate per `plan.md` §Testing). **Implemented as**: (a) added `iac/.terraform-docs.yml` (recursive inject mode, hide-empty, hide providers/requirements/modules); (b) added `<!-- BEGIN_TF_DOCS -->` / `<!-- END_TF_DOCS -->` markers to all 19 module READMEs (preserves hand-written prose; auto-generated Inputs/Outputs tables inside markers); (c) scaffolded minimal READMEs for 7 modules that previously lacked them (`container-app`, `container-apps-env`, `container-registry`, `cosmos-account`, `cosmos-canonical-store`, `identity`, `keyvault`); (d) ran `terraform-docs -c iac/.terraform-docs.yml iac` to populate; (e) wired a `terraform-docs/gh-actions@v1` step in `.github/workflows/iac-validate.yml` with `fail-on-diff: true` so future drift fails CI.
- [X] T127 Verify the `iac/environments/dev/` apply matches `quickstart.md` §A end-to-end: §A.1 login, §A.2 init, §A.3 plan (zero stateful destroys), §A.4 policy gate (all pass), §A.5 apply, §A.6 validate outputs + live dev URL still works + MSAL sign-in still works. **Capture wall-clock duration** of `tofu apply` and record it in the PR description; the SC-001 budget is 60 minutes for an empty-RG apply (this slice is an incremental retrofit, so the budget here is informational — but the timing data establishes a baseline for SC-001's later validation when a fresh env stands up via the test/prod templates, which is SC-007's 30-minute budget for an incremental env addition). **Verified locally 2026-05-30**: §A.1 `az login` OK against subscription `08b37dc0-…`; §A.2 `tofu init -reconfigure` with azurerm backend (rg `rg-busterminal-tfstate`, SA `btstatech0001`, container `tfstate`) successful; §A.3 plan completed in **22s** with `Plan: 34 to add, 8 to change, 2 to destroy` — both destroys are role-assignment replacements (`azurerm_cosmosdb_sql_role_assignment.developer_data_contributor`, `azurerm_role_assignment.pipeline_kv_secrets_officer`), neither is in `data-model.md` §3 stateful list; §A.4 `run-policies.sh` returned 6 pass · 0 fail (BT-IAC-002 SKIP per Q2c dev; BT-IAC-006 SKIP no --composition-dir); §A.5 apply deferred to CI's `iac-apply-dev.yml` on merge (standard flow — local apply bypasses CI gates); §A.6 live dev FE returns HTTP 200, backend reachable. Plan timing of 22s establishes the SC-001 baseline.
- [X] T128 Confirm Container Apps service-bus FQDN / AI Search endpoint / Cosmos endpoint outputs resolve correctly via `tofu output -json` and match the format documented in `contracts/outputs-contract.md`. **Verified 2026-05-30**: `iac/environments/dev/outputs.tf` declares the full contract surface (`service_bus_namespace_fqdn` ← `module.service_bus.fqdn`; `ai_search_endpoint` ← `module.ai_search.endpoint`; `cosmos_account_endpoint` ← `module.cosmos_account.account_endpoint`; `container_apps_environment_default_domain` ← `module.container_apps_env.default_domain`). Live state has only the 17 pre-spec-005 outputs (cosmos_account_endpoint already resolves to `https://cosmos-bt-dev-chdev01.documents.azure.com:443/`); the new outputs will materialize on the first post-spec-005 apply via CI.
- [X] T129 Confirm the BT-IAC-003-adjacent invariant from `quickstart.md` §D "Browser telemetry stops appearing" — App Insights `local_authentication_disabled` is `false`; browser SDK ingestion continues to work. **Verified 2026-05-30**: `iac/modules/monitoring/variables.tf:39-53` defines the var with default `false` and the documented rationale (Q1c / research §6 — JS SDK does not support AAD ingestion); dev composition does not override; live `appi-bt-dev` returns `properties.DisableLocalAuth = false` via `az resource show`.
- [X] T130 Update `speckit-artifacts/tech-stack.md` to reflect any durable new conventions this slice introduces (the `allLogs`-only diagnostic convention, the new policy-gate rule IDs, the new AVM pins) per CLAUDE.md §"When You're Unsure". **Implemented as**: added six new rows to §6 Hosting & Infrastructure (IaC diagnostics convention, IaC policy gates BT-IAC-001..007, IaC module docs via terraform-docs, IaC env posture per Q2c, stateful resource protection, networking-defaults clarifications) and one new row to §8 (IaC CI gates enumeration). AVM version pins are governed by the existing "AVM preferred / versions pinned explicitly" row.
- [X] T131 [P] Verify the `.terraform.lock.hcl` files in `iac/environments/dev/`, `iac/environments/test/`, `iac/environments/prod/`, `iac/platform-bootstrap/`, and each new module directory are committed per `research.md` §13. **Verified 2026-05-30**: lock files present + tracked for `environments/{dev,test,prod}` + `platform-bootstrap` + 18 of 19 modules (`naming` is pure-HCL with no provider dependencies — no lock file possible). Generated missing locks via `tofu init -backend=false` in 16 modules (`ai-search`, `app-registration-roles`, `container-apps-env`, `cosmos-account`, `cosmos-canonical-store`, `graph-permissions`, `identity`, `keyvault`, `monitoring`, `networking`, `probe-job-internal-caller`, `service-bus`, `workload-identity`, `container-app`, `container-registry`); the existing three (`diagnostic-settings`, `federated-credential`, `private-endpoint`) were already committed.
- [X] T132 Capture and resolve any `checkov` / `tfsec` findings introduced by this slice; either fix the finding or add an explicit skip with justification in the resource code. **Verified 2026-05-30**: `checkov -d iac` (source HCL framework) returns 47 pass · 0 fail; `checkov -f tfplan.json --framework terraform_plan` returns 12 pass · 0 fail; `tfsec iac/` returns 0 critical · 0 high · 0 medium · 0 low. All deferred findings are already documented in `iac/.checkov.yaml` skip-check with per-rule rationale. No new findings introduced by this slice.
- [X] T133 Run `/speckit-analyze` after `tasks.md` lands to verify cross-artifact consistency (spec ↔ plan ↔ tasks); resolve any CRITICAL/HIGH findings before `/speckit-implement`. **Verified 2026-05-30**: cross-artifact analysis produced 0 CRITICAL · 0 HIGH findings; 5 MEDIUM (all documented/accepted: ambiguity in FR-029 sizing now closed by research §10 cross-reference, "long-lived secrets" in FR-021 scope-deferred to downstream specs, FR-009 optionality closed by plan §Structure resolving to "extend existing container-app module", SC-001/SC-007 deferred by Q1c dev-only scope, FR-005/FR-038 wording refinement deferred); ~13 LOW items. 47/47 FRs have task coverage. Constitution alignment: 0 violations; one open follow-up (ADR for pipeline-MI subscription grant) tracked but blocked on the constitution-level ADR-location TODO.
- [X] T135 Author `specs/005-infrastructure-baseline/production-hardening.md` per FR-046 — an audit-grade reference enumerating every production hardening switch this slice introduces, separated from local/dev conveniences. Required sections: (a) Network posture (data-services public access disabled, PEs live, DNS zones linked); (b) Secrets posture (KV purge protection ON, soft-delete retention 90d, RBAC authorization); (c) State posture (storage account versioning + soft-delete, lifecycle.prevent_destroy on stateful resources); (d) Identity posture (workload UAMI role enumeration per FR-033, deployment-MI per-env federation per FR-032); (e) Observability posture (allLogs forwarded, retention default, AAD ingestion for backend); (f) CI posture (BT-IAC-001 through BT-IAC-007 active, BT-IAC-002 enforced for prod, BT-IAC-007 manual approval gate). For each item, list the controlling tf-var, the prod default, and the override mechanism. Cross-reference `contracts/policy-rules.md` and `quickstart.md` §C. **Implemented as**: `specs/005-infrastructure-baseline/production-hardening.md` covering all six required sections (A. Network · B. Secrets · C. State · D. Identity · E. Observability · F. CI) plus a "How to use this document" section. Each row records the switch, prod default, controlling Tofu variable + location (file:line), override mechanism, and enforcement (policy gate / module precondition / `.checkov.yaml` skip rationale). Cross-references `contracts/policy-rules.md`, `quickstart.md` §C, `data-model.md` §3, `iac/environments/prod/variables.tf`, and `iac/.checkov.yaml`.
