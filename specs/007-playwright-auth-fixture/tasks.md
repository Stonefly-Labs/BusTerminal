...
- [ ] T037 [US2] Document the `admin` persona's "provisioned-but-currently-unused" status in `web/tests/auth/README.md` (created in T046 below) — Admin exists for forthcoming admin-scoped tests; absence of an Admin-using spec today is intentional, not an omission
...
- [ ] T040 [US3] Edit `.github/workflows/ci.yml` — `frontend` job: (a) add an `azure/login@v2` step using the existing GH OIDC federation pattern (mirror the auth shape from `cd-dev.yml` or `iac-apply-dev.yml`); (b) add a step that resolves persona UPNs from `terraform output` (or from a fixed naming convention with `unique_suffix` resolved from a non-sensitive workflow input) and fetches the four passwords via `az keyvault secret show --vault-name ${{ env.DEV_KV_NAME }} --name e2e-test-user-<persona>-password --query value -o tsv`, exporting each as a `>> $GITHUB_ENV` line (which masks them in subsequent step logs); (c) replace the existing "Start backend in background (mock auth)" step's env vars with real-Entra values (`AzureAd__TenantId`, `AzureAd__ClientId`, `AzureAd__Audience` pulled from non-sensitive workflow vars or repo vars) and add `REGISTRY_PERSISTENCE=InMemory` and `AzureAd__UseMockHandler=false`; (d) leave the backend-readiness probe and Playwright invocation untouched
...
- **User Story 2 (Phase 4)**: Depends on US1 complete (extends globalSetup and fixture). T030, T031 sequential then `[P]` with each other. T032–T035 are `[P]`, each depends on T030–T031. T036 depends on T032–T035. T037 depends on T046 (the README that documents Admin).
