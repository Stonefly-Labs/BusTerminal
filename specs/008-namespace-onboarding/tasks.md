...
- [X] T143 [P] Implemented as `api/BusTerminal.Api.Tests/Features/Namespaces/NamespaceOpenApiContractTests.LiveOpenApi_DeclaresEveryContractOperation` — parses `contracts/namespace-onboarding-api.yaml`, asserts every (path, method) appears in the runtime `/openapi/v1.json` under the documented `/api` server prefix. Mirrors the spec-003 OpenApiConformanceTests pattern.
- [ ] T144 [P] Deferred — requires the deployed dev backend. Capture once Phase 5 + Phase 6 land in dev (cd-dev.yml roll-forward).
- [ ] T145 [P] Deferred — requires the deployed dev backend + Azure Monitor access; same gate as T144.
- [ ] T146 [P] Deferred — requires the deployed dev backend + a real ARM namespace + Reader grant; same gate as T093 / T115.
- [X] T147 [P] Runbook (`iac/runbooks/grant-namespace-reader.md`) covers all 7 sections (why / find principalId / grant / verify / rollback / attestation / references); the wizard sidebar's `<GrantReaderGuidance>` pulls runbook URL + sample command from `/api/namespaces/identity`. No additional artifacts needed.
- [X] T148 Updated `specs/003-auth-and-identity/contracts/role-permission-matrix.md` with a new "Cross-spec extensions (informative)" section listing `Namespace Administrator` (additive; does not alter core-role semantics).
- [X] T149 Updated `specs/006-service-bus-registry-core/contracts/audit-event.schema.json` — `eventType` enum now lists the five spec-008 values + new optional `lifecycleReason` field. Frontend `auditEventTypeSchema` in `web/lib/registry/schemas.ts` updated to match (T060 parity test passes).
- [X] T150 Updated `speckit-artifacts/tech-stack.md` §1 Backend with a new `Azure.ResourceManager.ServiceBus 1.x` row (managed-identity auth, no connection strings — FR-017 / FR-033 / SC-007).
- [X] T151 Confirmed via existing `OnboardedSourceGateTests` (4/4 passing) — covers spec-006 PUT/DELETE on Manual (200) + spec-006 PUT/DELETE on Onboarded (409 with redirect codes `OnboardedNamespaceWriteNotPermitted` and lifecycle/archive). Spec-006 frontend (`(authenticated)/registry/*`) is unchanged and continues to render Manual namespaces.
- [ ] T152 Open work — local backend tests 524/524, frontend 185/185, FE typecheck clean, FE lint 0 errors (down from 8). Next: push the branch, monitor PR #61 CI. IaC docs check passes locally after `terraform-docs` regeneration of `app-registration-roles/README.md`, `cosmos-registry-store/README.md`, `graph-permissions/README.md`.
- [ ] T153 [P] [TEST] Deferred — blocked on the MSW namespace-API harness gap that has been tracked since Phase 3 (same as T068/T099/T124/T125/T126). Track as a follow-up.
- [X] T154 [P] [TEST] Implemented as `NamespaceOpenApiContractTests.LiveOpenApi_NoDeleteOnAnyNamespaceRoute` — asserts the runtime OpenAPI document declares no DELETE operations on any `/api/namespaces/*` path (FR-026).
- [X] T155 [P] Added `scripts/check-no-servicebus-credentials.sh` (greps `.cs|.ts|.tsx|.tf|.json|.yaml|.yml|.md` for `Endpoint=sb://`, `SharedAccessKey=`, `SharedAccessKeyName=`, `SharedAccessSignature=`) and hooked it into `.github/workflows/ci.yml` `security` job after the gitleaks step. Local smoke pass: no matches.
