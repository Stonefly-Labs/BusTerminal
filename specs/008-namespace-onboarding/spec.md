...
- **FR-043**: Any added Azure resource role assignments — specifically the built-in `Reader` grant on `Microsoft.ServiceBus/namespaces/{name}` scope for BusTerminal's workload managed identity (per FR-014) — MUST be expressed in OpenTofu, parameterized per environment, and MUST follow the spec 005 BT-IAC-001..007 policy gates and the spec-005 Q5c `allLogs`-only diagnostic convention where applicable. The role assignment scope MUST be the namespace resource itself (not the resource group, not the subscription) so least-privilege is preserved.
