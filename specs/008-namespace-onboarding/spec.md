...
- Q: Should the `namespace-administrator` role apply tenant-wide or per-environment? → A: Tenant-wide single role — any holder may onboard/mutate/lifecycle/validate namespaces in any environment. Per-environment role scoping is reserved for a future governance spec. Pre-go-live tenant-population attestation (spec 006 FR-037) remains the upstream gate.
- Q: Which Azure RBAC role does BusTerminal's managed identity need on a target namespace to satisfy the `RequiredPermissions` validation check? → A: Built-in `Reader` role, granted at the `Microsoft.ServiceBus/namespaces/{name}` resource scope. Zero data-plane (send/receive/peek) permissions are granted. No custom role is authored.
- Q: What is the search scope of the Entra picker in wizard step 3 (and on the namespace ownership-edit UI)? → A: Entire configured Entra tenant. The picker searches all users AND all groups in BusTerminal's tenant via Microsoft Graph using application permissions `User.Read.All` + `Group.Read.All`. Operators MAY assign any tenant principal as primary owner, secondary owner, technical steward, or support contact — assigned principals do NOT need to be pre-onboarded as BusTerminal users.
- Q: Can an administrator persist a namespace in `PendingValidation` state when initial validation fails (e.g., resource not yet provisioned)? → A: No. Initial onboarding is hard-blocked at step 5 unless validation is Healthy or Degraded (Existence + Accessibility both pass). `PendingValidation` is a transient processing state, never reaches durable storage in this slice. Failed initial onboarding creates no namespace record, no audit event, and no `ValidationRun` record. The "provision-tomorrow" workflow is handled by re-opening the wizard after the Azure resource exists.
- Q: Does the inventory view expose a multi-select "re-validate all" batch action? → A: No. Re-validation is initiated exclusively from a single namespace's details page (one at a time). Inventory MAY surface last-validated timestamp and validation status but MUST NOT offer a batch-revalidate action. Async/batch validation patterns (parallelism, retry/backoff, ARM rate-limit handling) are deferred to a future ops/sync spec.
...
- **FR-013**: The Entra picker in step 3 (and on the ownership-edit UI per FR-020) MUST search the **entire configured Entra tenant** — all users and all groups — via Microsoft Graph using application permissions `User.Read.All` and `Group.Read.All`. The picker MUST support search by display name and email (users) or display name (groups), MUST distinguish users from groups visually, and MUST NOT restrict candidates to principals already granted access to BusTerminal's Enterprise App. Assigning a principal as owner/steward/support contact does NOT grant that principal access to BusTerminal itself — ownership is a metadata reference, not an access grant.
- **FR-013a**: The Graph application permissions `User.Read.All` and `Group.Read.All` required by FR-013 MUST be granted via tenant admin consent at deploy time. The deployment runbook MUST capture this as an attestation alongside the spec 006 FR-037 tenant-population attestation. Graph queries MUST use the application identity (not the signed-in user's delegated token) so the picker behaves consistently regardless of the operating user's directory privileges.

#### Connectivity & Authorization Validation

- **FR-014**: The validation step MUST execute five named checks against the supplied Azure Resource ID, each individually reported with `Pass | Fail | Skipped` and a human-readable reason: **Existence** (ARM `GET` returns the namespace resource), **Accessibility** (ARM call succeeds without auth error), **RequiredPermissions** (BusTerminal's workload managed identity holds the built-in Azure `Reader` role at the `Microsoft.ServiceBus/namespaces/{name}` resource scope, verified via an ARM permissions enumeration call; data-plane Service Bus roles such as `Azure Service Bus Data Receiver` MUST NOT be granted by this slice), **IdentityAuthorization** (token exchange completes; managed identity is correctly federated), **ApiReachability** (Service Bus management endpoint responds to a lightweight metadata probe).
...
- **FR-022**: The system MUST model `lifecycleStatus` with values `PendingValidation`, `Active`, `Disabled`, `Archived` (operational axis), independent of spec 006's `status` field (`Active`/`Deprecated` — governance axis). A namespace MAY simultaneously be `lifecycleStatus = Active` and `status = Deprecated`. `PendingValidation` is **transient-only** in this slice — it represents the in-flight state between step-5 register click and durable persistence, and MUST NOT appear on any record retrievable via the inventory or details APIs. The value is reserved in the enum for a future async-validation spec but emits from no code path that writes a durable document in this slice.
- **FR-023**: Permitted lifecycle transitions are: **initial create at `Active`** (on first successful step-5 register, when validation aggregate is `Healthy` or `Degraded`), `Active ⇄ Disabled` (administrator action with required reason note), `Active|Disabled → Archived` (administrator action with confirmation), `Archived → Disabled` (administrator restore action). Any other transition MUST be rejected.
- **FR-023a**: Initial onboarding MUST be hard-blocked at step 5 when the latest validation run's aggregate is `Unhealthy` (Existence or Accessibility fail per FR-016). On a hard block, the system MUST NOT persist a namespace record, an audit event, or a `ValidationRun` record — the wizard remains open with failures surfaced inline and the operator MAY retry validation or abandon the wizard. The "I'd like to capture the metadata now and validate later" workflow is explicitly out of scope for this slice; operators MUST wait until the Azure resource is provisioned and reachable, then onboard.
...
- **FR-032**: Namespace onboarding, metadata update, ownership change, lifecycle transition, and on-demand validation execution MUST all require the **namespace-administrator** role. Any authenticated tenant user (per spec 006 FR-037) MAY read the inventory and details views. This is a deliberate divergence from spec 006's "any authenticated tenant user may read and write" stance — onboarding decisions carry governance weight and MUST be gated. The role MUST be backed by a single **tenant-wide** Entra App Role granted via Enterprise App assignment (consistent with spec 003's identity foundation) and surfaced as an `IsNamespaceAdministrator()` check on the platform principal accessor. A holder of this role MAY act on namespaces in any environment — per-environment role scoping is explicitly out of scope for this slice and reserved for a future governance spec; the tenant-population attestation in spec 006 FR-037 remains the upstream gate.
...
- **FR-043**: Role assignments granting access to resources **within BusTerminal's own IaC inventory** MUST be expressed in OpenTofu, parameterized per environment, and MUST follow the spec 005 BT-IAC-001..007 policy gates and the spec-005 Q5c `allLogs`-only diagnostic convention where applicable. The built-in `Reader` grant on `Microsoft.ServiceBus/namespaces/{name}` scope for BusTerminal's workload managed identity (per FR-014) is **explicitly carved out** from this rule: operator-supplied namespaces are not in BusTerminal's IaC inventory and cannot be enumerated at plan time, so the Reader grant is performed out-of-band by the operator via the documented runbook (`iac/runbooks/grant-namespace-reader.md`). FR-014's `RequiredPermissions` check is the runtime verification surface, and the Reader role GUID (`acdd72a7-3385-48ef-bd42-f606fba81ae7`) is added to the pipeline MI RBAC-Admin condition allowlist to preserve forward optionality for an IaC-driven model. When granted (whether in OpenTofu for BusTerminal-owned resources or out-of-band for operator-supplied namespaces), the role assignment scope MUST be the namespace resource itself (not the resource group, not the subscription) so least-privilege is preserved.
...
- **SC-006**: 0% of onboarding attempts whose latest validation run is `Unhealthy` (Existence or Accessibility fail) result in *any* persisted namespace record, audit event, or `ValidationRun` document. Failed initial onboardings leave durable storage untouched; the wizard surfaces the failure and the operator may retry or abandon.
...
- **Batch / multi-select re-validation from the inventory**: Out of scope. Re-validation is single-namespace only, initiated from the details page (per FR-015, FR-018). Async/parallel validation runners with ARM rate-limit handling and retry/backoff are reserved for a future ops/sync spec.
