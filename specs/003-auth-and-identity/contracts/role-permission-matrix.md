...
|---|---|---|---|
| `Namespace Administrator` | Spec 008 (FR-032 clarified decision) | `BusTerminal.NamespaceAdministrator` | Required for all namespace-onboarding writes (POST /api/namespaces, PUT /api/namespaces/{id}/metadata, PUT /api/namespaces/{id}/ownership, POST /api/namespaces/{id}/lifecycle, POST /api/namespaces/{id}/validation-runs). Backend reads are AuthN-only. **Frontend `Read` operation class implicitly includes this role** so a user holding only `NamespaceAdministrator` can see the navigation entries for the surfaces they administer (a NamespaceAdministrator-only user without `Reader` would otherwise see an empty sidebar). |

Cross-spec roles are declared via `iac/modules/app-registration-roles/` in the same Entra app as the four core roles; the policy registration lives next to the slice that owns it (e.g., `BusTerminal.Api.Features.Namespaces._Shared.NamespacePolicies`). Adding a cross-spec role does NOT require a structural change to the matrix's five operation classes — the row above is a directory of in-tree extensions. A role MAY appear in the `Read` operation class as an implicit grant (the `NamespaceAdministrator` precedent) when the role's domain inherently requires reading the surfaces it acts on; any other cross-spec inclusion in the core matrix table requires a clarification.
