...
- [ ] T089 [P] Run the full quickstart `quickstart.md` end-to-end on a fresh machine; record any deviations and fix them in-place. Target: ≤ 30 min from clone to a working role-aware local stack. *(Deferred to user: requires a fresh machine to be meaningful — cannot be validated in the spec-implementer session. Carried as the only outstanding documentation/onboarding signal for slice 003. See `specs/003-auth-and-identity/POLISH-PUNCHLIST.md` § T089 for the verification protocol.)*
- [X] T090 [P] Update root `README.md` to mention the role-aware authentication posture and link to `docs/identity-and-secrets.md`, `docs/identity-role-administration.md`, `docs/identity-graph-permissions.md`. *(Stale "spec 002 active" pointer corrected to spec 003 in the same pass.)*
- [X] T091 [P] Create `docs/identity-role-administration.md` (the operator runbook promoted from `quickstart.md` Part B): how to grant/remove roles, how to bootstrap the initial Admin, how role propagation works. *(Five-section runbook: the four roles, Part A (bootstrap initial Admin from A.2.2), Part B (grant-role recurring action from quickstart Part B), Part C (propagation behavior), Part D (smoke-test a fresh grant against the probes). Cross-references identity-and-secrets.md, identity-graph-permissions.md, local-development.md, and the binding role-permission matrix.)*
- [X] T092 [P] Add Playwright smoke in `web/tests/e2e/no-access-experience.spec.ts` covering SC-008: a no-role test user sees `/no-access` rendered within 2 seconds of completing MSAL sign-in. **Include an explicit Playwright timing assertion** — capture a timestamp at the post-MSAL-redirect navigation event, then `await expect(page.getByTestId('no-access-page')).toBeVisible({ timeout: 2000 })` and assert `(Date.now() - postRedirectTs) <= 2000`. Fail the test if the page renders late. *(Authored as `test.fixme` pending the MSAL E2E auth fixture (see T093 caveat). Both the `toBeVisible({ timeout: 2000 })` poll and the wall-clock `Date.now()` assertion are in place; the timestamp is captured at the `framenavigated` event that commits the no-access URL.)*
- [X] T093 [P] Add Playwright smoke in `web/tests/e2e/msal-sign-in-and-whoami.spec.ts` covering the basic sign-in → `/whoami` → effective-roles-rendered → sign-out cycle. **Also verify the inherited (002) posture is intact post-rewrite**: (a) the page loads over HTTPS in the deployed env (assert `page.url().startsWith('https://')` when `BASE_URL` is non-localhost); (b) a deliberately-malformed bearer token sent to `/whoami` via `fetch` returns `401` with a `WWW-Authenticate: Bearer ...` header (confirms the inherited Microsoft.Identity.Web validation pipeline is still active after T028). *(Split into two cases: the sign-in cycle is `test.fixme` until a dev-tenant MSAL fixture lands; the malformed-bearer 401+WWW-Authenticate assertion runs unconditionally — it deliberately sends a bogus token so it provides a live CI signal today. HTTPS-posture and `traceparent` regex assertions are wired into the fixme'd block per T093 + T096 specs.)*
- [X] T094 [P] Add a contract conformance test in `api/BusTerminal.Api.Tests/Integration/OpenApiConformanceTests.cs` that parses `contracts/role-probes.openapi.yaml` + `contracts/whoami.openapi.yaml` and compares against the live OpenAPI document emitted by the API. Fail on shape drift. *(Path+method conformance only — minimum-viable drift detector that doesn't drag YamlDotNet in. Hand-rolled YAML extractor (one regex for paths, one for methods) keyed to the indent style the contracts already use; live OpenAPI fetched from `MapOpenApi()`'s `/openapi/v1.json` via a dev-mock `WebApplicationFactory`. Asserts every contract-declared (path, method) pair exists on the live API. Test passes against current main: 1/1 green; full suite 96/96.)*
- [ ] T095 Run `axe` against the new no-access page and role-aware affordances (T043, T048); resolve any WCAG 2.2 AA violation. *(Deferred to user: requires a running dev stack and the `axe` CLI / browser extension. The static assertions in the no-access page (`aria-labelledby`, `aria-disabled`, semantic landmarks) and role-aware-button (`aria-disabled`, `role-aware-button-disabled` testid) carry the structural posture. See `specs/003-auth-and-identity/POLISH-PUNCHLIST.md` § T095 for the run protocol.)*
- [X] T096 [P] Verify W3C Trace Context (`traceparent`/`tracestate`) propagation on MSAL-acquired requests (constitution-bound). Add a manual verification step to `quickstart.md` Part D and an automated assertion to T093. *(Quickstart Part D gains a "W3C Trace Context — `traceparent` propagation" subsection with a 5-step DevTools verification protocol. Automated regex assertion (`^00-[0-9a-f]{32}-[0-9a-f]{16}-[0-9a-f]{2}$`) lives in the T093 spec's fixme'd block; activates with the MSAL fixture.)*
- [X] T097 [P] Update `speckit-artifacts/tech-stack.md` § 7 (Identity & Authentication) to mention MSAL for SPA auth and reference the role-permission matrix as the binding contract for future slices. This is the durable home for the slice's identity decisions. *(§ 7 now itemizes nine standards: MSAL human sign-in, Microsoft.Identity.Web token validation, UAMI service-to-service auth, OIDC federation for pipelines, `DefaultAzureCredential` via a single factory, role-based access control bound to the role-permission matrix, Microsoft Graph posture, embedded-credential prohibition with gitleaks gate, local-dev `az login` flow.)*
- [ ] T098 Run `tofu plan` against `iac/environments/dev/` and assert it shows only the additive changes expected by this slice (new modules, role definitions, Graph permission grant) plus zero-effect refactors (`moved` blocks). No destructive changes. *(Deferred to PR CI: `tofu validate` against the dev composition is clean locally (with `-backend=false`). Full `tofu plan` requires the dev azurerm backend + OIDC-federated pipeline credentials + per-env `TF_VAR_*` values, which are CI-scoped — the iac-validate workflow's `plan-dev` job is the authoritative signal. See `specs/003-auth-and-identity/POLISH-PUNCHLIST.md` § T098 for the assertion list to validate in the PR's plan comment.)*
- [X] T099 [P] Verify SC-002 / SC-007: re-run `gitleaks detect --redact --no-banner --source=.` after all polish tasks land; confirm zero findings. Document the run output in the PR description. *(Run locally on the polish branch: `109 commits scanned`, `73.89 MB`, `no leaks found`. Output captured here for the PR description.)*
- [X] T100 Update `docs/architecture.md` (inherited from 002) with a short "Identity & Authorization" section showing the MSAL → API → role-policy flow and the workload-identity → API call shape. One diagram + a paragraph; defer to the role-permission matrix for details. *(Deviation: `docs/architecture.md` did not actually exist — the "inherited from 002" framing was a mis-statement in the original task description. Created the file fresh with a Component Map header table, the requested Identity & Authorization section (two flow diagrams + commentary), and a Cross-cutting / Where-to-read-next pair of closing sections. The role-permission matrix is referenced as the binding contract.)*
- [X] T101 [P] Add a CI guard in `.github/workflows/iac-validate.yml` (or a new `iac-credential-scan` job) that fails the build on any new occurrence of inline Azure-service connection-string / account-key / SAS-token patterns inside `iac/` HCL files. Use a `grep -E` against the file set with patterns like `(AccountKey|SharedAccessKey|connection_string|primary_access_key|sas_token)\s*=` and exclude documented exceptions via an inline allowlist. Future-proofs FR-015's "no static credentials" stance as later slices introduce Cosmos / AI Search / Storage / OpenAI / Service Bus / App Configuration resources. *(Mirrors the slice-005 inline-IAM lint script's shape: new `scripts/lint-iac-inline-credentials.sh` denylists nine attribute names (`AccountKey`, `SharedAccessKey`, `connection_string`, `primary_access_key`, `secondary_access_key`, `primary_connection_string`, `secondary_connection_string`, `sas_token`, `shared_access_policy_key`); skips `#`/`//` comment lines; carries an empty allowlist with the policy that each future entry needs a one-line rationale. Wired into the `validate` job in `.github/workflows/iac-validate.yml` after the existing inline-IAM lint step. Local run: PASS.)*
