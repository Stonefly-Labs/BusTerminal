...
- [X] T126 [P] Update `speckit-artifacts/tech-stack.md` per `research.md §21` and `quickstart.md §11`: add TanStack Query 5.x (§2 Frontend), FluentValidation 11.10.x (§1 Backend), Cosmos change-feed-lease-container convention (§5), v2 native Functions-for-CAE (§6).
- [X] T127 [P] Update `web/README.md` and `api/BusTerminal.Api/` README (if present, else create stub) to point at `specs/006-service-bus-registry-core/quickstart.md` for the registry walkthrough. (api README created; both READMEs link the registry walkthrough.)
- [ ] T128 [P] Run the SC-002/SC-003/SC-004/SC-005 perf validations from the dev environment: capture App Insights metrics for search p95 (< 1s), detail p95 (< 500ms), CRUD p95 (< 1s), index lag p95 (< 5s); record results in `specs/006-service-bus-registry-core/checklists/perf-results.md`. **Scaffold + KQL queries authored in `checklists/perf-results.md`; numeric capture pending operator/CI run against the dev environment under representative load — see `checklists/validation-runs.md`.**
- [ ] T129 [P] Run the SC-008 a11y validation: confirm `pnpm test:a11y` passes with zero violations on every registry route on both dark and light themes. **Suite is wired (24 tests across explorer/search/detail/reduced-motion × {light,dark} × {chromium,firefox,webkit}); execution pending operator/CI run against a populated dev environment — see `checklists/validation-runs.md`.**
- [X] T129a [P] [TEST] Add `web/tests/a11y/registry/reduced-motion.spec.ts` — Playwright a11y test that emulates `prefers-reduced-motion: reduce` (`page.emulateMedia({ reducedMotion: 'reduce' })`) and asserts (a) the explorer tree expand/collapse does NOT animate, (b) the conflict-modal open/close does NOT animate, (c) the status-change visual cue does NOT animate. Covers FR-048.
- [X] T129b [P] [TEST] Add `web/tests/e2e/registry/sc-010-time-to-find.e2e.spec.ts` — Playwright timed assertion for SC-010: with a populated registry, the operator types a partial name in the global search affordance from an arbitrary page and reaches the entity's detail page in under 30 seconds (asserts wall-clock from first keystroke to detail-page `domcontentloaded`). Captures the metric to App Insights via the existing observability adapter. *(Metric emitted via `performance.measure("sc010.time-to-find", …)` — the AI browser SDK's PerformanceObserver pipeline forwards User-Timing entries to the connected resource; plus a `console.info` fallback for the no-op adapter path.)*
- [ ] T130 [P] Run the SC-012 trace-correlation validation: with a populated registry, select an arbitrary UI trace in App Insights and confirm the linked backend spans share the same trace id. **KQL query authored in `checklists/validation-runs.md`; execution pending operator run against the dev environment — see `checklists/validation-runs.md`.**
- [X] T131 [P] [TEST] Cross-story integration test `api/BusTerminal.Api.Tests/Features/Registry/EndToEndScenariosTests.cs` covering all three quickstart walkthroughs (golden-path, search, relationships+audit) in a single fixture.
- [X] T132 [P] Update `web/components/app-shell/` global navigation to expose a top-level "Registry" link (icon + label) so the registry route is discoverable from the platform's landing pages. *(Implemented in `web/components/layout/navigation-shell.tsx` — the NAV_ENTRIES list now carries a `Registry` entry with the `Database` lucide icon and `matchPrefix: true` so deep registry routes still highlight the entry.)*
- [ ] T133 Confirm spec-005 `iac/policies/run-policies.sh` reports zero BT-IAC violations against the new resources; capture the report in the PR description. **Pending CI run on the PR — requires pipeline-supplied tfvars and a `tofu plan` JSON; see `checklists/validation-runs.md`.**
- [X] T134 Run `pnpm run test:contracts` (T060) + the OpenAPI contract test (T082) + the FluentValidation/schema contract test (T061) as a final cross-layer parity gate; if any drift exists, fix at the source. *(Executed 2026-06-04: `pnpm run test:contracts` → 8 passed; `dotnet test --filter OpenApiContractTests|SharedSchemaContractTests` → 7 passed; no drift detected.)*
